...
function convert(csv, headerCount, delimiter, stringDelimiter, lineEndCharacter  ) {
	if (!csv) {
		...
		message+="Usage - convert(csv, headerCount, delimiter, stringDelimiter, lineEndCharacter"
	...
	for ( var headerIndex=0; headerIndex<headerCount;headerIndex++) {
		JSONTable.headerRows.push(parseStringIntoArrayOfValues(lineArray[headerIndex], delimiter, stringDelimiter))
	}
	
	for ( var rowIndex = headerIndex; rowIndex<lineArray.length; rowIndex++ ) {
		JSONTable.rows.push(parseStringIntoArrayOfValues(lineArray[rowIndex], delimiter, stringDelimiter))
	}
...
function extractFields(stringToExtractIntoFields, delimiter, stringDelimiter) {
	if (!stringToExtractIntoFields||!delimiter||!stringDelimiter) {
		var message = "usage: extractFields(stringToExtractIntoFields, delimiter, stringDelimiter)" 
	...
	for ( var index = 0; index<stringToExtractIntoFields.length; index++) {
		...
		else if(stringToExtractIntoFields.charAt(index)==stringDelimiter && !inString) {
			inString=true
		}
		else if(stringToExtractIntoFields.charAt(index)==stringDelimiter && inString) {
			inString=false
	...
	if ( fieldValue.length>0 || stringToExtractIntoFields.charAt(stringToExtractIntoFields.length-1)==delimiter) {
		fieldArray.push(convertFieldToValue(fieldValue))
	}
